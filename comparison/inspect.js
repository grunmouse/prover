...
const primordials = require('./inspect/primordials');
const {
  internalBinding,
  Array,
  ArrayIsArray,
...
const {
  ...
  join,
  removeColors,
} = require('./inspect/internal/util');

const {
  isStackOverflowError,
} = require('./inspect/internal/errors');
...
const {
  ...
  isBooleanObject,
  isBigIntObject,
} = require('./inspect/internal/util/types');

const assert = require('./inspect/internal/assert');

const { BuiltinModule } = require('./inspect/internal/bootstrap/realm');
const {
  validateObject,
  validateString,
} = require('./inspect/internal/validators');
...
function inspect(value, opts) {
	//Создаёт контекст и вызывает formatValue
  // Default options
  const ctx = {
    budget: {},
    indentationLvl: 0,
...
function formatValue(ctx, value, recurseTimes, typedArray) {
	//Обрабатывает null, Proxy, вызывает customInspect, проверят рекурсивные ссылки,
	//в остальных случаях вызывает formatRaw
